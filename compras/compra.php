...
$sql = "select tb_compras.*,tb_fornecedor.FOR_NOME,tb_tipopagto.TPG_DESCRICAO 
from tb_compras 
inner join tb_fornecedor on (tb_compras.FOR_CODIGO=tb_fornecedor.FOR_CODIGO)
inner join tb_tipopagto on (tb_compras.TPG_CODIGO=tb_tipopagto.TPG_CODIGO)
where COM_CODIGO = (select max(COM_CODIGO) from tb_compras)";

$sql = mysqli_query ($con, $sql) or die ("Erro na sql! sql");
$dados = mysqli_fetch_array($sql);


$sql2 = "select * from tb_procompras";
$sql2 = mysqli_query ($con, $sql2) or die ("Erro na sql! sql2");


$sql3 = "select tb_itenscompras.*,tb_procompras.*,tb_compras.COM_CODIGO 
from tb_itenscompras 
inner join tb_procompras on (tb_itenscompras.PCO_CODIGO=tb_procompras.PCO_CODIGO) 
inner join tb_compras on (tb_itenscompras.COM_CODIGO=tb_compras.COM_CODIGO) 
 where tb_itenscompras.COM_CODIGO = (select max(COM_CODIGO) from tb_compras)";
$sql3 = mysqli_query ($con, $sql3) or die ("Erro na sql! sql3");

$sql4 = "select SUM(tb_itenscompras.ITC_QTDE*tb_itenscompras.ITC_VRUNITARIO) AS valortotalcompra
from tb_itenscompras
inner join tb_compras on (tb_itenscompras.COM_CODIGO=tb_compras.COM_CODIGO) 
where tb_itenscompras.COM_CODIGO = (select max(COM_CODIGO) from tb_compras)";
$sql4 = mysqli_query ($con, $sql4) or die ("Erro na sql! sql4");
        ...
        <div class="row wrapper border-bottom white-bg page-heading">
          ...
          <div class="row">
			...
			$(document).ready(function(){
				$("select[name='sel_produto']").blur(function(){
					var $valor = $("input[name='txt_vrunitario']");
					$.getJSON('function.php',{ 
						sel_produto: $( this ).val() 
					},function( json ){
						$valor.val( json.sel_produto );
					});
				});
			});
	        	</script>
                    <!--Cadastrar item da venda-->
            <form method="post" style="height:100%;" action="../compras/add_item_compra.php">
                    
            <div class="form-group  row" style="margin-top:2%;"><label class="col-sm-2 col-form-label">Código</label>
                <div class="col-sm-10"><input type="text" class="form-control" name="txt_codigo" value="<?php echo $dados['COM_CODIGO'];?>" readonly></div>
            </div>
            <div class="form-group  row" style="margin-top:2%;"><label class="col-sm-2 col-form-label">Fornecedor</label>
                <div class="col-sm-10"><input type="text" class="form-control" name="txt_fornecedor" value="<?php echo $dados['FOR_NOME'];?>" readonly></div>
            </div>
            <div class="form-group  row" style="margin-top:2%;"><label class="col-sm-2 col-form-label">Tipo de pagamento</label>
            	<div class="col-sm-10"><input type="text" class="form-control" name="txt_tipopagto" value="<?php echo $dados['TPG_DESCRICAO'];?>" readonly></div>
            </div>
            <div class="form-group  row" style="margin-top:2%;"><label class="col-sm-2 col-form-label">Observação</label>
            	<div class="col-sm-10"><input type="text" class="form-control" name="txt_obs" value="<?php echo $dados['COM_OBSERVACAO'];?>" readonly></div>
            </div>
                     
            <div class="form-group  row" style="margin-top:2%;"><label class="col-sm-2 col-form-label">Produto para compra</label>
            	<div class="col-sm-10"><select class="form-control m-b" name="sel_produto" >
                                <?php while($dados2=mysqli_fetch_array($sql2)){?>
                                        <option value="<?php echo $dados2['PCO_CODIGO'];?>"><?php echo $dados2['PCO_DESCRICAO'];?></option>
                                <?php } ?>
                                    </select></div>
            </div>
            <div class="form-group  row" style="margin-top:2%;"><label class="col-sm-2 col-form-label">Valor Unitário</label>
            	<div class="col-sm-10"><input type="text" class="form-control" name="txt_vrunitario"></div>
            </div>
            <div class="form-group  row" style="margin-top:2%;"><label class="col-sm-2 col-form-label">Quantidade</label>
            	<div class="col-sm-10"><input type="text" class="form-control" name="txt_qtde"></div>
            </div>
            <div class="hr-line-dashed"></div>

                                    <div class="col-sm-offset-2" style="float:right;">
                                        
                                        <button class="btn btn-primary btn-sm" type="submit">Add Item</button><br><br>
                ...
                <!--Consulta -->
                    <div class="ibox">
                        <div class="ibox-content">
                                    ...
                                    <div class="full-height-scroll">
                                        <div class="table-responsive">
                                            <table class="table table-striped table-hover">
                                                ...
                                                <tr>
                                                    <td><?php echo $i++?></td>
                                                    <td><?php echo $dados3['PCO_DESCRICAO'];?></td>
                                                    <td><?php echo 'R$ ' .$dados3['ITC_VRUNITARIO'];?></td>
                                                    <td><?php echo $dados3['ITC_QTDE'];?></td>
                                                    <td></td>
                                                    <td><?php echo 'R$ ' .$dados3['ITC_QTDE']*$dados3['ITC_VRUNITARIO'];?></td>
                                                    <td><a href="delete.php?ID=<?php echo $dados3['ITC_CODIGO']; ?>&&compra=<?php echo $dados3['COM_CODIGO']; ?>"> 
                            ...
                            <div class="form-group  row" style="margin-top:2%;"><label class="col-sm-2 col-form-label">Total da compra</label>
            	                 <div class="col-sm-10"><input type="text" class="form-control" name="txt_totalcompra" value="<?php echo 'R$ ' .$dados4['valortotalcompra'];?>"></div>
                             </div>
                             
                             <div class="col-sm-offset-2" style="float:right;">
                                        
                                    <button class="btn btn-primary btn-sm" type="submit">Finalizar compra</button><br><br>
                                 
                             </div>
                             <input type="hidden" class="form-control" name="txt_codigo" value="<?php echo $dados['COM_CODIGO'];?>">
                             </form>
                </div>
            </div>
